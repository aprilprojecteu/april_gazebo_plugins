...
void FlashLightPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  node->Init();
  std::cout << "I will subscribe now" << std::endl;
  gazebo::transport::SubscriberPtr sub = node->Subscribe("flash_plugin_topic_sub", &FlashLightPlugin::cb, this);
}

//////////////////////////////////////////////////
void FlashLightPlugin::cb(const std::string &msg)
{
  std::cout << "i heard a flash messsage" << std::endl;
}
